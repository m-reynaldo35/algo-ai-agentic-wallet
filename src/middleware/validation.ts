 ...
 */
export function validateSandboxExport(sandboxExport: SandboxExport): ValidationResult {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    // Check if this is the toll transaction
    if (axfer.assetIndex === USDC_ASSET_ID) {
      tollCount++;

      if (axfer.amount !== EXPECTED_TOLL) {
        errors.push(
          `Rule 1: Toll amount mismatch on txn [${i}]. Expected ${EXPECTED_TOLL}, got ${axfer.amount}`,
        );
      } else if (axfer.receiver.toString() !== TREASURY_ADDRESS) {
        errors.push(
  ...
  if (tollCount === 0) {
    ...
    errors.push(`Rule 1: Expected exactly 1 USDC toll transfer, found ${tollCount}`);
  }
