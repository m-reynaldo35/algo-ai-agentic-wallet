    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="space-y-2">
          {events.map((e, i) => {
            const isPolicyBreach = e.failureReason === "POLICY_BREACH";
            const isSuccess = e.event === "settlement.success";
            ...
            return (
            <div key={i} className={`border rounded-lg p-4 font-mono text-sm ${cardClass}`}>
              <div className="flex items-center justify-between">
                <span className={labelClass}>
                  {isPolicyBreach ? "policy.breach" : e.event}
                  {isPolicyBreach && <span className="ml-2 text-xs bg-amber-900/60 text-amber-300 px-1.5 py-0.5 rounded">TEAL REJECTED</span>}
                </span>
